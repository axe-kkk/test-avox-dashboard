...
const stepTypeColors = {
  ...
  wait: 'bg-amber-50 border-amber-200 text-amber-700',
...
function SequenceRow({ seq, isSelected, onClick }: { seq: Sequence; isSelected: boolean; onClick: () => void }) {
  ...
  return (
    <tr
      className={cn('hover:bg-slate-50 cursor-pointer transition-colors', isSelected && 'bg-blue-50/60')}
      onClick={onClick}
    >
      <td className="px-5 py-4">
        <div>
          <p className="text-sm font-semibold text-slate-900">{seq.name}</p>
          <p className="text-xs text-slate-500 mt-0.5">{seq.trigger}</p>
        </div>
      </td>
      ...
      <td className="px-4 py-4">
        <div className="flex items-center gap-1.5 text-sm">
          ...
          <span className="font-semibold text-slate-900">{seq.enrolledGuests}</span>
        </div>
      </td>
      <td className="px-4 py-4">
        <div className="flex items-center gap-2">
          ...
          <span className="text-xs font-semibold text-slate-700">{(seq.completionRate * 100).toFixed(0)}%</span>
        </div>
      </td>
    ...
    <div className="flex h-full overflow-hidden">
      <div className="flex-1 flex flex-col overflow-hidden">
        ...
        <div className="px-6 py-4 bg-white border-b border-slate-100 flex items-center gap-3 flex-shrink-0">
          <h1 className="text-base font-semibold text-slate-900 flex-1">Sequences</h1>
          <Button size="sm" variant="primary" onClick={() => addToast({ type: 'info', title: 'Builder opened', message: 'New sequence draft created' })}>
            <Plus className="w-3.5 h-3.5" /> New Sequence
          </Button>
        </div>

        {/* Stats row */}
        <div className="px-6 py-3 bg-white border-b border-slate-50 flex items-center gap-6">
          {[
            ...
            <thead className="border-b border-slate-100 sticky top-0 bg-white z-10">
              <tr>
                {['Sequence', 'Engine', 'Status', 'Enrolled', 'Completion', 'Last Run', ''].map(h => (
                  <th key={h} className="px-4 py-3 text-left text-[11px] font-semibold text-slate-400 uppercase tracking-wider first:px-5">
                    {h}
                  </th>
                ))}
              </tr>
      ...
      {selectedSeq && (
        <div className="w-[520px] flex-shrink-0 border-l border-slate-100 bg-white flex flex-col overflow-hidden">
          <div className="px-6 pt-5 pb-4 border-b border-slate-100 flex-shrink-0">
            <div className="flex items-start justify-between mb-2">
              <div>
                <h2 className="text-sm font-semibold text-slate-900">{selectedSeq.name}</h2>
                <p className="text-xs text-slate-500 mt-0.5">Trigger: {selectedSeq.trigger}</p>
              </div>
              <div className="flex items-center gap-2">
                <Badge variant={selectedSeq.status === 'active' ? 'success' : selectedSeq.status === 'paused' ? 'warning' : 'default'}>
                  {selectedSeq.status}
                </Badge>
                <button onClick={() => setSelectedSeq(null)} className="text-slate-400 hover:text-slate-600 text-lg">×</button>
              </div>
            </div>
            <div className="grid grid-cols-3 gap-3">
              <div className="bg-slate-50 rounded-xl p-3">
                <p className="text-[10px] text-slate-400 mb-0.5">Enrolled</p>
                <p className="text-lg font-bold text-slate-900">{selectedSeq.enrolledGuests}</p>
              </div>
              <div className="bg-slate-50 rounded-xl p-3">
                <p className="text-[10px] text-slate-400 mb-0.5">Completion</p>
                <p className="text-lg font-bold text-slate-900">{(selectedSeq.completionRate * 100).toFixed(0)}%</p>
              </div>
              <div className="bg-slate-50 rounded-xl p-3">
                <p className="text-[10px] text-slate-400 mb-0.5">Steps</p>
                <p className="text-lg font-bold text-slate-900">{selectedSeq.steps.length}</p>
              </div>
            </div>
          </div>

          <div className="flex-1 overflow-y-auto p-6">
            <h3 className="text-xs font-semibold text-slate-400 uppercase tracking-wider mb-4">Sequence Flow</h3>
              ...
              {selectedSeq.steps.map((step, i) => (
                <div key={step.id} className="flex flex-col items-start">
                  ...
                  {i < selectedSeq.steps.length - 1 && (
                    <div className="w-0.5 h-4 bg-slate-200 ml-[68px] flex-shrink-0" />
                  )}
                </div>
              ))}
            </div>
            ...
            <div className="mt-5 pt-5 border-t border-slate-100">
              <h3 className="text-xs font-semibold text-slate-400 uppercase tracking-wider mb-3">Enrollment</h3>
              <div className="bg-slate-50 rounded-xl p-4 space-y-3">
                <div className="flex justify-between items-center text-xs">
                  <span className="text-slate-500">Enrollment Type</span>
                  <span className="font-medium text-slate-800">Automatic — Trigger-based</span>
                </div>
                <div className="flex justify-between items-center text-xs">
                  <span className="text-slate-500">Trigger</span>
                  <span className="font-medium text-slate-800">{selectedSeq.trigger}</span>
                </div>
                <div className="flex justify-between items-center text-xs">

...
export const overviewTopActions: {
  ...
  {
    type: "Follow-up after abandoned",
    engine: "Conversion",
    count: 286,
    connects: 1144,
    share: 3.9,
...
export const connectsDaily = days(8, 14).map((d, i) => ({
  ...d,
  connects:
    820 + Math.round(Math.sin(i * 0.6) * 220 + Math.cos(i * 1.1) * 80 + i * 14),
}));
...
export const ENGINE_ACTION_TYPES: Record<
  EngineName,
  { type: string; connects: number }[]
> = {
  Conversion: [
...
export const engineDailyMix: Record<
  EngineName,
  { date: string; [k: string]: number | string }[]
> = {
  Conversion: days(8).map((d, i) => ({
    ...d,
  ...
  Upsell: days(8).map((d, i) => ({
    ...d,
    sent: 32 + Math.round(Math.cos(i) * 8),
    accepted: 12 + (i % 4),
    declined: 8 + (i % 3),
  })),
  Arrival: days(8).map((d, i) => ({
    ...d,
...
export const upsellRevenueDaily = days(8).map((d, i) => ({
  ...d,
...
export const conciergeEscalations = [
  {
    question: "Is parking available for an oversized vehicle?",
    count: 8,
    lastSeen: "2h ago",
  },
  {
    question: "Do you accommodate kosher dietary requirements?",
    count: 6,
    lastSeen: "5h ago",
  },
  {
    question: "Can the spa accommodate a wheelchair?",
    count: 5,
    lastSeen: "1d ago",
  },
  {
    question: "Do you have an EV charger on-site?",
    count: 4,
    lastSeen: "1d ago",
  },
  {
    question: "Pet policy for emotional support animals?",
    count: 3,
    lastSeen: "2d ago",
  },
];
...
export const channelsDaily = days(8).map((d, i) => ({
  ...d,
...
export const operatorsDaily = days(8).map((d, i) => ({
  ...d,
...
export const loadHourly = Array.from({ length: 24 }).map((_, h) => ({
  ...
  total: Math.round(
    Math.exp(-Math.pow((h - 14) / 5, 2)) * 48 + Math.random() * 6,
  ),
  ai: Math.round(Math.exp(-Math.pow((h - 14) / 5, 2)) * 36 + Math.random() * 4),
  agents: Math.round(
    Math.exp(-Math.pow((h - 14) / 5, 2)) * 12 + Math.random() * 3,
  ),
...
export const scheduledReports = [
  {
    ...
    frequency: "Weekly",
    recipients: ["ceo@hotel.com", "gm@hotel.com"],
    lastRun: "21 Apr 2026",
  ...
  {
    id: "r-3",
    name: "Monthly Connects billing",
    frequency: "Monthly",
    recipients: ["finance@hotel.com"],
    lastRun: "01 Apr 2026",
    status: "active" as const,
  },
  {
    id: "r-4",
    name: "Conversion engine deep-dive",
    frequency: "Weekly",
  ...
  },
  {
    ...
    frequency: "Daily",
    recipients: ["quality@hotel.com", "gm@hotel.com"],
    lastRun: "22 Apr 2026",
    status: "active" as const,
  },
];

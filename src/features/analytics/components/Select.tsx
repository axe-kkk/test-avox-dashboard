...
interface Props<V extends string> {
  ...
  options: Option<V>[];
  onChange: (v: V) => void;
  /** Prefix shown on the trigger when an option is selected, e.g. "Engine". */ prefix?: string;
  /** Optional left-side icon for the trigger. */ icon?: ReactNode;
  width?: number;
  align?: "left" | "right";
}
    ...
    <div className="relative" ref={ref}>
      ...
      >
        ...
        <span className="truncate text-strong">
          {" "}
          {prefix && (
            <span className="text-subtle font-normal">{prefix}: </span>
          )}{" "}
          {current?.label}{" "}
        </span>{" "}
        <ChevronDown
          className={cn(
            "w-3 h-3 text-subtle transition-transform flex-shrink-0",
            open && "rotate-180",
          )}
        />{" "}
      </button>{" "}
      {open && (
        ...
        >
          {" "}
          {prefix && (
            <p className="px-2 py-1.5 text-[10px] font-semibold text-subtle ">
              {prefix}
            </p>
          )}{" "}
          {options.map((o) => {

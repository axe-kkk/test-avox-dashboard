...
export function Funnel({ stages }: { stages: Stage[] }) {
  ...
  return (
    <div className="grid items-stretch gap-2" style={{ gridTemplateColumns: `repeat(${stages.length}, 1fr)` }}>
      {stages.map((s, i) => {
        ...
        return (
          <div key={s.stage} className="relative">
            <div className="rounded-xl border border-brand-border p-4 bg-white h-full flex flex-col">
              <p className="text-[10px] font-semibold text-subtle uppercase tracking-[0.14em] mb-3">
                Stage {i + 1}
              </p>
              <p className="text-[14px] font-semibold text-strong leading-tight">{s.stage}</p>

              <div className="mt-3 mb-3 h-1.5 bg-surface-3 rounded-full overflow-hidden">
                <div
                  className={cn('h-1.5 rounded-full', i === 0 ? 'bg-brand-blue' : 'bg-brand-blue-light')}
                ...
                <div className="text-right">
                  {s.connects != null && (
                    <p className="text-[11px] text-muted tabular-nums">
                      {s.connects.toLocaleString()}<span className="text-subtle"> conn.</span>
                    </p>
                  )}
                  {s.conv != null && i > 0 && (

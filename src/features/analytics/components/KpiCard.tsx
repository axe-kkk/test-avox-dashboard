...
interface Props {
  ...
  invertDelta?: boolean;
  accent?: boolean;
  /** 0..1 — renders a progress bar instead of trend. Pairs with `hint`. */
  progress?: number;
}

export function KpiCard({
  label, value, hint, delta, invertDelta, accent, progress,
}: Props) {
  const positiveTrend =
  ...
  return (
    ...
    >
      ...
      {accent && (
        <span className="absolute inset-x-0 top-0 h-[2px] bg-brand-blue/80" aria-hidden />
      )}
      ...
      {progress != null ? (
        <div className="mt-2.5">
          <div className="h-[3px] bg-surface-3 rounded-full overflow-hidden">
            <div
              className={cn('h-full rounded-full', accent ? 'bg-brand-blue' : 'bg-strong')}
        ...
        <p className="mt-2 text-[11px] leading-tight truncate">
          ...
          {delta != null && hint && <span className="text-faint mx-1.5">·</span>}
          {hint && (

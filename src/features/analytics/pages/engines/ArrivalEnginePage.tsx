...
export function ArrivalEnginePage() {
  ...
  return (
    ...
    >
      ...
      <SectionCard title="Arrival funnel" subtitle="Pre-arrival → Preferences → Check-in → Key → Welcome">
        ...
        <table className="w-full">
          <thead>
            <tr className="border-b border-brand-border">
              {['Guest', 'Check-in', 'Pre-arrival', 'Preferences', 'Online check-in', 'Digital key', 'Conn.'].map((h, i) => (
                <th key={h} className={`py-2.5 text-[10px] font-semibold text-subtle uppercase tracking-[0.14em] ${i < 2 ? 'text-left' : i === 6 ? 'text-right' : 'text-center'}`}>
                  {h}
                </th>
              ))}
            </tr>
          ...
          <tbody className="divide-y divide-border-soft">
            {arrivalGuests.map(g => (
              <tr key={g.name} className="hover:bg-surface-2 transition-colors">
                <td className="py-2.5 text-[12px] text-strong font-medium">{g.name}</td>
                <td className="py-2.5 text-[12px] text-muted tabular-nums">{g.date}</td>
                <CheckCell ok={g.pre} />
                <CheckCell ok={g.prefs} />
                <CheckCell ok={g.checkin} />
                <CheckCell ok={g.key} />
                <td className="py-2.5 text-[12px] text-strong tabular-nums font-semibold text-right">{g.connects}</td>
              </tr>
            ))}
          </tbody>
        </table>

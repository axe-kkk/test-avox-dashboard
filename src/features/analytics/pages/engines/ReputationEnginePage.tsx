...
import { type Period, reputationDaily, reputationActionsByType, reputationLog } from '../../lib/mockData';
    ...
    >
      ...
      <SectionCard title="Average score" subtitle="Daily moving average">
        <ResponsiveContainer width="100%" height={220}>
          <LineChart data={reputationDaily} margin={{ top: 4, right: 4, left: 0, bottom: 0 }}>
            <CartesianGrid strokeDasharray="3 3" stroke="#F4F5F7" vertical={false} />
            <XAxis dataKey="date" tick={axisTick} axisLine={false} tickLine={false} />
            <YAxis domain={[3.5, 5]} tick={axisTick} axisLine={false} tickLine={false} width={32} />
            <Tooltip {...chartTooltipStyle} />
            <Line type="monotone" dataKey="score" stroke="#2355A7" strokeWidth={2.5} dot={{ fill: '#2355A7', r: 3 }} />
          </LineChart>
        </ResponsiveContainer>
      </SectionCard>
      ...
      <SectionCard title="Actions by type">
        <ResponsiveContainer width="100%" height={220}>
          <BarChart data={reputationActionsByType} margin={{ top: 4, right: 4, left: 0, bottom: 0 }}>
            <CartesianGrid strokeDasharray="3 3" stroke="#F4F5F7" vertical={false} />
            <XAxis dataKey="type" tick={{ fontSize: 11, fill: '#5C6370' }} axisLine={false} tickLine={false} />
        ...
        <table className="w-full">
          <thead>
            <tr className="border-b border-brand-border">
              {['Date', 'Guest', 'Score', 'Channel', 'Platform', 'Reply'].map(h => (
          ...
          <tbody className="divide-y divide-border-soft">
            {reputationLog.map((r, i) => (
              <tr key={i} className="hover:bg-surface-2 transition-colors">
                ...
                <td className="py-2.5">
                  <span className="inline-flex items-center gap-0.5">
                    {Array.from({ length: 5 }).map((_, k) => (
                      <Star
                        key={k}
                        className={cn('w-3 h-3', k < r.score ? 'fill-brand-blue text-brand-blue' : 'text-faint')}
                      />
                    ))}
                  </span>
                </td>
                ...
                <td className="py-2.5">
                  <span className={cn('inline-flex h-5 px-2 items-center rounded-full text-[10px] font-semibold border capitalize', STATUS_BADGE[r.status])}>
                    {r.status}
                  </span>
                </td>
              </tr>

...
import { type Period, engineDailyMix, conciergeSplit, conciergeEscalations } from '../../lib/mockData';
import { useApp } from '../../../../app/AppContext';

const LOG = [
  { date: '21 Apr 2026, 14:32', type: 'AI guest Q&A',          guest: 'Élise Caron',    reservation: 'RSV-10248', channel: 'WhatsApp', connects: 3 },
  { date: '21 Apr 2026, 13:18', type: 'Service booking',        guest: 'Henrik Bauer',   reservation: 'RSV-10239', channel: 'Web Widget', connects: 5 },
  { date: '21 Apr 2026, 11:54', type: 'In-stay message sent',   guest: 'Mei Tanaka',     reservation: 'RSV-10231', channel: 'WhatsApp', connects: 2 },
  { date: '21 Apr 2026, 10:22', type: 'Recommendations sent',   guest: 'Diego Fernández',reservation: 'RSV-10218', channel: 'WhatsApp', connects: 2 },
  { date: '21 Apr 2026, 09:48', type: 'Escalation to reception',guest: 'Theo Lambert',   reservation: 'RSV-10204', channel: 'WhatsApp', connects: 4 },
];
      ...
      >
        <ResponsiveContainer width="100%" height={260}>
          <BarChart data={data} margin={{ top: 4, right: 4, left: 0, bottom: 0 }}>
            ...
            <Bar dataKey="rec"        name="Recommendations" stackId="a" fill="#5C6370" />
            <Bar dataKey="booking"    name="Bookings"        stackId="a" fill="#BED4F6" />
            <Bar dataKey="instay"     name="In-stay"         stackId="a" fill="#C4C8CF" />
            <Bar dataKey="escalation" name="Escalations"     stackId="a" fill="#0E1013" radius={[6, 6, 0, 0]} />
          </BarChart>
        </ResponsiveContainer>
      </SectionCard>

      <div className="grid grid-cols-2 gap-5">
        <SectionCard title="AI resolved vs Escalated" subtitle="Where the engine handed off">
          <div className="grid grid-cols-[1fr_180px] items-center gap-4">
            <ResponsiveContainer width="100%" height={220}>
              <PieChart>
                <Pie data={conciergeSplit} dataKey="value" nameKey="label" innerRadius={56} outerRadius={88} paddingAngle={2} strokeWidth={0}>
                  {conciergeSplit.map(s => <Cell key={s.label} fill={s.color} />)}
                </Pie>
                <Tooltip {...chartTooltipStyle} />
              </PieChart>
            </ResponsiveContainer>
              ...
              {conciergeSplit.map(s => {
                ...
                return (
                  <div key={s.label}>
                    ...
                    <p className="text-[10px] text-subtle mt-1 tabular-nums">{s.value} actions</p>
                  </div>
                );
              })}
            </div>
            ...
            {conciergeEscalations.map((e, i) => (
              <div key={i} className="flex items-start gap-3 py-2.5 group">
                <div className="flex-1 min-w-0">
                  <p className="text-[12px] text-strong font-medium">{e.question}</p>
                  <p className="text-[10px] text-subtle mt-0.5">Asked {e.count} times · last {e.lastSeen}</p>
                </div>
                <button
                  onClick={() => addToast({ type: 'success', title: 'Q&A draft created — review in Knowledge Base' })}
        ...
        <table className="w-full">
          <thead>
            <tr className="border-b border-brand-border">
              {['Date', 'Type', 'Guest', 'Reservation', 'Channel', 'Conn.'].map((h, i) => (
                <th key={h} className={`py-2.5 text-[10px] font-semibold text-subtle uppercase tracking-[0.14em] ${i === 5 ? 'text-right' : 'text-left'}`}>{h}</th>
              ))}
            </tr>
          </thead>
          <tbody className="divide-y divide-border-soft">
            {LOG.map((l, i) => (
              <tr key={i} className="hover:bg-surface-2 transition-colors">
                <td className="py-2.5 text-[12px] text-muted tabular-nums">{l.date}</td>
                <td className="py-2.5 text-[12px] text-strong font-medium">{l.type}</td>
                <td className="py-2.5 text-[12px] text-muted">{l.guest}</td>
                <td className="py-2.5 text-[12px] text-muted tabular-nums">{l.reservation}</td>
                <td className="py-2.5 text-[12px] text-muted">{l.channel}</td>
                <td className="py-2.5 text-[12px] text-strong tabular-nums font-semibold text-right">{l.connects}</td>
              </tr>
            ))}
          </tbody>
        </table>
